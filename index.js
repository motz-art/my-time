...
exports.default = downloadObjectAsJson;
/* eslint-env browser */
...
var _timeIco = require('./time-ico.js');

var _timeIco2 = _interopRequireDefault(_timeIco);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

const notificationPeriod = 300000; /* eslint-env browser */
...
function notify(task, remainingTime) {
  if (remainingTime <= 0 && lastNotificationTask !== task.id) {
    ...
    nextNotificationTime = calculateNextNotificationTime(remainingTime);
    showNotification(task.title, remainingTime > 0 ? `${task.remainingTimeFormatted} left.` : `Task is ${task.remainingTimeFormatted} overdue.`, task);
  }
}
...
var _formatTime = require('./format-time.js');
...
function updateTime(task) {
  ...
  task.remainingTimeFormatted = (0, _formatTime2.default)(remain);
...
function createTask({
  ...
  const remainingTimeFormatted = (0, _formatTime2.default)(remainingTime);
  ...
  task.getRemainingTimeFormatted = function (short) {
    return (0, _formatTime2.default)(task.getRemainingTime(), short);
  };
...
var _parseTaskInput = require('./parse-task-input');

var _parseTaskInput2 = _interopRequireDefault(_parseTaskInput);

var _task = require('./task.js');

var _task2 = _interopRequireDefault(_task);

var _tasks = require('./tasks.html');
...
_vue2.default.component('tasks', {
  ...
  computed: {
    completedPlaned: function () {
      return this.tasks.reduce((sum, task) => sum + (task.isComplete ? task.duration : 0), 0);
    },
  ...
  created: async function () {
    const tasksData = await _storage2.default.loadTasks();
  ...
  methods: {
    ...
    createTask: function (taskData) {
      ...
      taskModel.addEventListener('tic', e => _notification2.default.notify(e.task, e.remainingTime));
      if (taskModel.startTime) {
        _notification2.default.setupNotifications(taskModel);
      }
      return taskModel;
    },
    ...
    saveTask: function ({ task }) {
      ...
      return _storage2.default.saveTask(data).then(() => {
        task.id = data.id;
      });
    },
    ...
    remove: async function (task) {
      ...
      await _storage2.default.removeTask(task.getData());
    },
    addTask: async function () {
      const info = (0, _parseTaskInput2.default)(this.taskName);
...
function draw({ remainingTime, totalTime, remainingTimeFormatted }, options) {
  ...
  ctx.lineTo(canvas.width / 2 + Math.cos(startDeg) * bigRadius, canvas.height / 2 + Math.sin(startDeg) * bigRadius);
  ctx.fillStyle = '#888';
  ctx.fill();
  ...
  ctx.fillText(remainingTimeFormatted, (canvas.width - size.width) / 2, canvas.height / 2 + 5);
